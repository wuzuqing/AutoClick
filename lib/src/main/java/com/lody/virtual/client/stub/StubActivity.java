...
public abstract class StubActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		super.onCreate(null);
		Log.d(TAG, "onCreate: StubActivity");
		finish();
		...
		if (r.intent != null) {
			if (TextUtils.equals(r.info.processName, VirtualRuntime.getProcessName()) && r.userId == VUserHandle.myUserId()) {
				...
				Log.d(TAG, "onCreate: Retry to inject the HCallback to instead of the exist one."+intent.getPackage());
				...
				Log.d(TAG, "onCreate: Start the target Activity in other process."+r.intent.getPackage() );
